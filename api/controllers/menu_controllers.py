...
class MenusController(MethodView):
    ...
    def post(self):
        ...
        elif not self.DataValidation.validate_name(self.food_item):
            return ResponseErrors.invalid_name()
        elif self.DataValidation.check_item_name(self.food_item):
            return ResponseErrors.item_already_exists()

        food_data = self.food_model.add_food_item(self.food_item.lower())
